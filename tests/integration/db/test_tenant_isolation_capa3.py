"""Issue #16 — Aislamiento cross-tenant (RN-D01) sobre
`contracts`/`contract_adjustments`.

Mismo patron que `tests/integration/db/test_tenant_isolation_capa2.py`
(issue #14, tablas `properties`/`property_service_accounts`): ejerce el
aislamiento end-to-end con `set_tenant_context`, no solo inspecciona el
catalogo (eso lo cubre `test_capa3_contratos.py`).

SDD: core/sdd_02_domain_model.md §3 RN-D01
     + docs/skills/tenant-isolation.md
Implements: CA-16 (RLS aisla cross-tenant en ambas tablas de la Capa 3)
"""
...
@pytest.fixture
async def two_orgs_with_one_contract_and_adjustment_each() -> AsyncGenerator[None]:
    """Crea dos organizaciones, cada una con landlord + property + renter +
    contract + un ajuste `pending` (conexion por default: `adminprop`,
    superuser con BYPASSRLS -- set up ajeno al aislamiento que se quiere
    probar). Limpieza al final."""
    ...
    adjustment_a, adjustment_b = uuid.uuid4(), uuid.uuid4()
    ...
    async with session_factory() as session, session.begin():
        ...
        await session.execute(
            sa.text(
                ...
                "VALUES (:id_a, :org_a, :contract_a, :due_period, 'pending'), "
                "(:id_b, :org_b, :contract_b, :due_period, 'pending')"
            ),
            {
                "id_a": str(adjustment_a),
                "org_a": str(ORG_A),
                "contract_a": str(contract_a),
                "id_b": str(adjustment_b),
                "org_b": str(ORG_B),
                "contract_b": str(contract_b),
                "due_period": date(2026, 4, 1),
            },
        )
    yield
    async with session_factory() as session, session.begin():
        ...
        await session.execute(sa.text("SET LOCAL ROLE adminprop_superadmin"))
        await session.execute(
            sa.text("DELETE FROM contract_adjustments WHERE organization_id IN (:org_a, :org_b)"),
...
async def test_ca_16_tenant_a_solo_ve_sus_propias_filas(
    table: str, two_orgs_with_one_contract_and_adjustment_each
):
    """RN-D01: con `app.current_tenant_id = ORG_A`, `adminprop_app` solo ve
    la fila de la organizacion A, tanto en `contracts` como en
...
@pytest.mark.parametrize("table", ["contracts", "contract_adjustments"])
async def test_ca_16_tenant_b_solo_ve_sus_propias_filas(
    table: str, two_orgs_with_one_contract_and_adjustment_each
):
...
@pytest.mark.parametrize("table", ["contracts", "contract_adjustments"])
async def test_ca_16_sin_contexto_seteado_no_ve_ninguna_fila(
    table: str, two_orgs_with_one_contract_and_adjustment_each
):
...
@pytest.mark.parametrize("table", ["contracts", "contract_adjustments"])
async def test_ca_16_contexto_limpiado_a_none_no_ve_ninguna_fila_ni_revienta(
    table: str, two_orgs_with_one_contract_and_adjustment_each
):
...
@pytest.mark.parametrize("table", ["contracts", "contract_adjustments"])
async def test_ca_16_adminprop_superadmin_bypassa_rls_y_ve_ambos_tenants(
    table: str, two_orgs_with_one_contract_and_adjustment_each
):
