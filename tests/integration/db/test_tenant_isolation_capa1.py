"""Issue #12 — Aislamiento cross-tenant (RN-D01) sobre `landlords`/`renters`.

Mismo patron que `tests/integration/db/test_tenant_isolation_capa0.py`
(issue #5, tabla `roles`): ejerce el aislamiento end-to-end con
`set_tenant_context`, no solo inspecciona el catalogo (eso lo cubre
`test_capa1_personas.py`).

SDD: core/sdd_02_domain_model.md §3 RN-D01
     + docs/skills/tenant-isolation.md
Implements: CA-12-02 (RLS aisla cross-tenant en ambas tablas)
"""
...
@pytest.fixture
async def two_orgs_with_one_landlord_and_renter_each() -> AsyncGenerator[None]:
    """Crea dos organizaciones, un landlord y un renter por cada una
    (conexion por default: `adminprop`, superuser con BYPASSRLS — set up
    ...
    async with session_factory() as session, session.begin():
        ...
        await session.execute(
            sa.text(
                ...
                "VALUES (:org_a, 'Landlord A', 10), (:org_b, 'Landlord B', 10)"
            ),
            {"org_a": str(ORG_A), "org_b": str(ORG_B)},
        )
        await session.execute(
            sa.text(
                "INSERT INTO renters (organization_id, name) "
                "VALUES (:org_a, 'Renter A'), (:org_b, 'Renter B')"
            ),
            {"org_a": str(ORG_A), "org_b": str(ORG_B)},
        )
    yield
...
async def test_ca_12_02_tenant_a_solo_ve_sus_propias_filas(
    table: str, two_orgs_with_one_landlord_and_renter_each
):
    """RN-D01: con `app.current_tenant_id = ORG_A`, `adminprop_app` solo ve
    la fila de la organizacion A, tanto en `landlords` como en `renters`."""
    session_factory = get_session_factory()
    async with session_factory() as session, session.begin():
...
@pytest.mark.parametrize("table", ["landlords", "renters"])
async def test_ca_12_02_tenant_b_solo_ve_sus_propias_filas(
    table: str, two_orgs_with_one_landlord_and_renter_each
):
...
@pytest.mark.parametrize("table", ["landlords", "renters"])
async def test_ca_12_02_sin_contexto_seteado_no_ve_ninguna_fila(
    table: str, two_orgs_with_one_landlord_and_renter_each
):
...
@pytest.mark.parametrize("table", ["landlords", "renters"])
async def test_ca_12_02_contexto_limpiado_a_none_no_ve_ninguna_fila_ni_revienta(
    table: str, two_orgs_with_one_landlord_and_renter_each
):
...
@pytest.mark.parametrize("table", ["landlords", "renters"])
async def test_ca_12_02_adminprop_superadmin_bypassa_rls_y_ve_ambos_tenants(
    table: str, two_orgs_with_one_landlord_and_renter_each
):
    ...
    async with session_factory() as session, session.begin():
        ...
        result = await session.execute(
            sa.text(
                f"SELECT organization_id FROM {table} WHERE organization_id IN (:a, :b)"
            ),
