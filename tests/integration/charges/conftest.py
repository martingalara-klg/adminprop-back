"""Fixtures compartidas de tests/integration/charges (issue #28).
    ...
    class Seeder:
        ...
        async def create_role(
            ...
            *,
            ...
            role_id = uuid.uuid4()
            permissions = permissions if permissions is not None else ["charge:manage"]
            session_factory = get_session_factory()
            async with session_factory() as session, session.begin():
                await session.execute(
                    sa.text(
        ...
        # ─── helpers propios de charges (issue #28) ────────────────────────
            ...
            async with session_factory() as session, session.begin():
                await session.execute(
                    sa.text(
                        ...
                        "(id, organization_id, recurring_charge_id, period, amount, notes, created_by) "
                        "VALUES (:id, :org_id, :recurring_charge_id, :period, :amount, :notes, :created_by)"
                    ),
                    {
        ...
        async def get_charge_entry(self, charge_entry_id: uuid.UUID) -> dict:
            session_factory = get_session_factory()
            async with session_factory() as session:
                result = await session.execute(
                    sa.text("SELECT amount, notes FROM charge_entries WHERE id = :id"),
                    {"id": str(charge_entry_id)},
                )
                row = result.mappings().one()
                return {"amount": str(row["amount"]), "notes": row["notes"]}
