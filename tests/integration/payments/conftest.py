"""Fixtures compartidas de tests/integration/payments (issue #22).

Mismo patron de engine/session-factory fresco por test que
tests/integration/contracts/conftest.py (evita "Future attached to a
...
`auth`, `superadmin`, `administracion`, `people`, `properties` y `contracts`.
"""
    ...
    class Seeder:
        ...
        async def create_organization_with_system_roles(
            ...
            *,
            ...
            permiso `payment:*`/`rent-period:*` (RN-A01). `grace_day`
            permite a los tests de CA-04-05 (dia de gracia) fijar un valor
            distinto al default (10) sin depender de `administracion`."""
        ...
        # ─── helpers propios de cobranzas (issue #22) ──────────────────────
        ...
        async def create_contract_row(
            ...
            *,
            ...
            """Siembra un `contract` directamente en DB -- los tests de
            cobranzas necesitan un contrato `active` ya existente, sin
            pasar por el flujo completo de alta/activacion del modulo de
            contratos (fuera del alcance del issue #22)."""
        ...
        async def create_rent_period_row(
            ...
            *,
            ...
            """Siembra un `rent_period` directamente en DB -- equivalente a
            lo que generaria el job `generate_rent_periods` (issue #21),
            sin depender de correrlo en cada test de este modulo."""
